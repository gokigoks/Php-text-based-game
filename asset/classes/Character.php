<?php
	...
	class Character implements battle, Object{
		protected $character_id, $current_equip, $current_weapon,$user_id,$defense,$curr_hp,$max_hp, $attack,$msg,$current_stage,$class;
		protected $user,$weapon_name,$critical;
		...
		public function __construct(){
				...
				$this->current_weapon = $this->user->getWeapon();
		...
		public function getStats(){
			...
			$query = "SELECT character_str, character_dex, character_int ,class from character_table";
			...
			if(!isset($this->weapon_name))
				{
					...
					$query.= "WHERE weapon_id = :weapon";
					$stmt = $pdo->prepare($query);
					$stmt->execute($this->current_weapon);
		...
		*/
		...
		public function calculateAttack(){
			...
			public function attack(Object $target){
				...
				$attack = $this->calculateAttack() + $this->getCritical();
				...
				{	
					...
					if($target->getCurrentHp()==0 || $target->getCurrentHp()<0){
						...
						$this->msg =  $target->getName().' has been defeated!! you get '.$reward_gold.' gold pieces!!<br>';
						$this->msg .="<a href='scene".$this->getCurrentStage().".php'>proceed journey</a>";
						
					}	
					
					$this->msg = $this->user->getUsername().' attacked and dealt '.$attack.' damage';
